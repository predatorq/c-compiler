...
%token T_Void T_Int T_While T_If T_Else T_Return T_Break T_Continue T_Print T_ReadInt T_Le T_Ge T_Eq T_Ne T_And T_Or T_IntConstant T_StringConstant T_Identifier
...
_Args:
    T_Int T_Identifier              { args[argc++] = ($2); }
|   _Args ',' T_Int T_Identifier    { args[argc++] = ($4); }
;

Vars:
    _Vars                           { write_func_head(); }
;
...
Var:
    T_Int T_Identifier              { vars[varc++] = ($2); }
|   Var ',' T_Identifier            { vars[varc++] = ($3); }
;
...
If:
    T_If            { istack[++itop] = ++ii; out_asm("_begIf_%d:", istack[itop]); }
;

Then:
    /* empty */     { out_asm("\tjz _elIf_%d", istack[itop]); }
;
...
EndIf:
    /* empty */     { out_asm("_endIf_%d:", istack[itop]); itop--; }
;
...
While:
    T_While         { wstack[++wtop] = ++ww; out_asm("_begWhile_%d:", wstack[wtop]); }
;

Do:
    /* empty */     { out_asm("\tjz _endWhile_%d", wstack[wtop]); }
;
...
BreakStmt:
    T_Break ';'     { out_asm("\tjmp _endWhile_%d", wstack[wtop]); }
;

ContinueStmt:
    T_Continue ';'  { out_asm("\tjmp _begWhile_%d", wstack[wtop]); }
;
